...
Phase 10: PIN Authentication
    ...
    - Backend: Create /api/auth/login endpoint (verify PIN, return session token)
    - Backend: Add middleware to check session token on protected endpoints
    - Frontend: Create PIN entry screen (startup and after timeout)
    - Frontend: Add session management (store token, refresh, logout)
    - Frontend: Add "Lock" button to nav bar for manual lock
    - Deliverable: PIN-based access control
